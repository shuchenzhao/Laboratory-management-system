...
public class ClassAndStudent_manage extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ClassAndStudent_manage frame = new ClassAndStudent_manage(new User());
	 ...
	 * Create the frame.
	 */
	public ClassAndStudent_manage(User user) {
		setTitle("\u73ED\u7EA7\u4E0E\u5B66\u751F\u7BA1\u7406");//界面名字
		...
		contentPane.add(btnNewButton_2);//录入
		
		btnNewButton_3 = new JButton("查询");
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				setVisible(false);//班级与学生管理界面不可见
				Select frame = new Select(user);
				frame.setVisible(true);//查询界面可见
			}
		});
		btnNewButton_3.setFont(new Font("仿宋", Font.PLAIN, 32));
		btnNewButton_3.setBackground(Color.WHITE);
		btnNewButton_3.setBounds(214, 335, 328, 78);
		contentPane.add(btnNewButton_3);//查询
